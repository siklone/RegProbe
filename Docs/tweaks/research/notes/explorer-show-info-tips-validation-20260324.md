# Explorer ShowInfoTip Validation (2026-03-24)

## Setting

- Key: `HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced`
- Value: `ShowInfoTip`
- Type: `REG_DWORD`

## Meaning

- `0` = hide Explorer info tips
- `1` = show Explorer info tips

Source: Microsoft Open Specifications, `GlobalFolderOptionsVista`, `showInfoTip`.

## 25H2 Dump Corroboration

```text
Docs/tweaks/_source-mirrors/regkit/assets/defaults/HKCU25H2.reg
"ShowInfoTip"=dword:00000001
```

## Procmon Runtime Evidence

```text
Source file:
H:\Temp\vm-tooling-staging\showinfotip-result.txt

State 0:
Explorer.EXE RegQueryValue HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\ShowInfoTip
SUCCESS Type: REG_DWORD, Length: 4, Data: 0

State 1:
Explorer.EXE RegQueryValue HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\ShowInfoTip
SUCCESS Type: REG_DWORD, Length: 4, Data: 1

Restore:
RESTORED_VALUE=1
```

## Raw Hit Files

```text
H:\Temp\vm-tooling-staging\showinfotip-0-hits.csv
H:\Temp\vm-tooling-staging\showinfotip-1-hits.csv
```

## Notes

- Explorer was restarted after each state change.
- The probe confirmed live Explorer consumption for both 0 and 1 on Win25H2Clean.
- The 25H2 default hive aligns with the documented enabled state (`1`).
