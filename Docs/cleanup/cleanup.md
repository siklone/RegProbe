...
| <a id="cleanup.memory-dumps"></a> `cleanup.memory-dumps` | Clear Memory Dump Files | Deletes %WINDIR%\\MEMORY.DMP and files under %WINDIR%\\Minidump. | Safe | `engine/Tweaks/Commands/Cleanup/ClearMemoryDumpFilesTweak.cs#L11` |
...
| <a id="cleanup.product-key"></a> `cleanup.product-key` | Remove Product Key from Registry | Runs slmgr.vbs /cpky to remove the Windows product key from the registry. Rollback does not restore the key automatically. | Advanced | `engine/Tweaks/Commands/Cleanup/RemoveProductKeyTweak.cs#L14` |
