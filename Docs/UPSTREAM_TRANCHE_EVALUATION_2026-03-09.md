...
| Memory management internals | `LargeSystemCache`, `PagedPoolSize`, `NonPagedPoolSize`, `RegistryQuota` | Can materially affect memory behavior and are not broadly suitable as generic desktop tweaks | Expert-only at most; not a default configuration item |
