...
- `scripts/vm-kvm/run-guest-app-publish-deploy-smoke.py`
  full host-side app smoke pipeline; runs `dotnet publish` for `app/app.csproj`, zips the publish output, hands the archive to the KVM deploy smoke wrapper, and reports publish, zip, guest deploy, and startup stages in one JSON summary; use `--verify-only` to emit readiness, blockers, an operator checklist, and the exact next commands without building or touching the VM, or `--dry-run` to print the planned publish/zip/deploy paths and commands without touching the VM
...
1. `README.md`
2. `Docs/research/vm-workflow.md`
3. `CONTRIBUTING.md`
4. this file
