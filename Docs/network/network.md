...
| `WwanSvc` | This service manages mobile broadband (GSM & CDMA) data card/embedded module adapters and connections by auto-configuring the networks. Systems that use mobile broadband devices normally run this service. |
...
| --- | --- | --- | --- | --- |
| <a id="network.disable-active-probing"></a> `network.disable-active-probing` | NCSI Active Probing Policy | Windows checks internet connectivity by making active test requests. Turning those tests off can change how Windows decides whether a net... | Medium | `research/records/network.disable-active-probing.review.json` |
| <a id="network.disable-default-shares"></a> `network.disable-default-shares` | Automatic Administrative Shares | Windows automatically creates hidden administrative shares such as C$ and ADMIN$. This setting decides whether those built-in shares are... | Medium | `research/records/network.disable-default-shares.json` |
| <a id="network.disable-ipv6"></a> `network.disable-ipv6` | IPv6 Stack Disable Override | IPv6 is the newer network protocol stack that Windows expects to have available. Turning it off can break some network features even if a... | Medium | `research/records/network.disable-ipv6.json` |
| <a id="network.disable-llmnr"></a> `network.disable-llmnr` | Link-Local Multicast Name Resolution (LLMNR) | LLMNR lets Windows ask nearby devices for names on the local subnet when normal DNS does not help. Modern networks often disable it, but... | Medium | `research/records/network.disable-llmnr.json` |
| <a id="network.disable-lltd"></a> `network.disable-lltd` | Set LLTD Policies to Default Behavior | LLTD is part of the older Windows network discovery stack. This tweak disables the explicit LLTD mapper and responder policies so Windows... | Medium | `research/records/network.disable-lltd.review.json` |
| <a id="network.disable-mdns"></a> `network.disable-mdns` | Set mDNS Policy to Local Settings | mDNS is a local-network name resolution protocol used by some discovery and service-advertising workflows. This policy setting disables t... | Medium | `research/records/network.disable-mdns.review.json` |
...
| <a id="network.disable-netbios-resolution"></a> `network.disable-netbios-resolution` | DNS Client NetBIOS Name Resolution | Windows can use NetBIOS-style name resolution through the DNS client policy layer. This policy lets you fully disable it, allow it, or ch... | Medium | `research/records/network.disable-netbios-resolution.json` |
| <a id="network.disable-plaintext-smb-passwords"></a> `network.disable-plaintext-smb-passwords` | Plaintext SMB Passwords | This setting decides whether Windows is willing to send an unencrypted password to certain old SMB servers. Modern systems should almost... | Medium | `research/records/network.disable-plaintext-smb-passwords.json` |
...
| <a id="network.disable-smb1"></a> `network.disable-smb1` | SMBv1 Server Protocol Support | SMB is the file-sharing protocol Windows uses for network shares. SMBv1 is the old version. Modern systems should avoid it, but some very... | Medium | `research/records/network.disable-smb1.json` |
| <a id="network.disable-smb2"></a> `network.disable-smb2` | SMBv2 and SMBv3 Server Protocol Support | SMBv2 and SMBv3 are the modern Windows file-sharing protocols. Turning them off forces the machine away from the normal modern SMB stack. | Medium | `research/records/network.disable-smb2.json` |
| <a id="network.disable-wifi-sense"></a> `network.disable-wifi-sense` | Wi-Fi Sense Suggested Hotspot Policy | This controls whether Windows can automatically connect to suggested open hotspots, contact-shared networks, and some paid hotspot discov... | Medium | `research/records/network.disable-wifi-sense.json` |
| <a id="network.enable-lltd-responder"></a> `network.enable-lltd-responder` | LLTD Responder Driver Policy | The LLTD Responder lets other PCs discover this device on the local network during Windows network mapping and related discovery tasks. | Medium | `research/records/network.enable-lltd-responder.json` |
| <a id="network.enable-lltdio"></a> `network.enable-lltdio` | LLTD Mapper I/O Driver Policy | LLTD helps Windows map and discover devices on a local network. The Mapper I/O side lets the PC ask the network about its topology. | Medium | `research/records/network.enable-lltdio.json` |
...
| <a id="network.prefer-ipv4"></a> `network.prefer-ipv4` | IPv4 Preference Override | This keeps IPv6 available but tells Windows to prefer IPv4 when both protocols can work. | Medium | `research/records/network.prefer-ipv4.json` |
| <a id="network.require-ntlm-ssp-client-session-security"></a> `network.require-ntlm-ssp-client-session-security` | Require NTLM SSP Client Session Security and 128-bit Encryption | This setting raises the minimum session security requirements for NTLM SSP based clients. In the app's enabled state it requires both NTL... | Medium | `research/records/network.require-ntlm-ssp-client-session-security.review.json` |
...
| <a id="network.smb-enable-large-mtu"></a> `network.smb-enable-large-mtu` | SMB Client Large MTU | Large MTU lets SMB send larger requests over the network. On modern Windows networks this usually helps file-copy and share performance. | Medium | `research/records/network.smb-enable-large-mtu.json` |
...
| <a id="network.smb-enable-quic"></a> `network.smb-enable-quic` | SMB over QUIC | SMB over QUIC lets Windows use SMB through QUIC instead of normal TCP in supported scenarios. It is a newer transport option and not a no... | Medium | `research/records/network.smb-enable-quic.review.json` |
| <a id="network.smb-encrypt-data"></a> `network.smb-encrypt-data` | SMB Server Encryption Requirement | SMB encryption protects file-sharing traffic so other systems on the network cannot easily read it in transit. Turning this on at the ser... | Medium | `research/records/network.smb-encrypt-data.json` |
| <a id="network.smb-increase-client-metadata-cache"></a> `network.smb-increase-client-metadata-cache` | SMB Client Metadata Cache Size Bundle | These values control how much SMB client metadata Windows caches and how many outstanding SMB requests it allows. | Medium | `research/records/network.smb-increase-client-metadata-cache.json` |
| <a id="network.smb-reject-unencrypted-access"></a> `network.smb-reject-unencrypted-access` | SMB Server Reject Unencrypted Access | This setting makes the SMB server reject clients that try to access shares without encryption. It is stricter than simply preferring safe... | Medium | `research/records/network.smb-reject-unencrypted-access.json` |
| <a id="network.smb-require-dialect-3_1_1"></a> `network.smb-require-dialect-3_1_1` | Require SMB Dialect 3.1.1 | This setting forces both SMB client and SMB server components to use only SMB 3.1.1. It is a strict compatibility boundary and can quickl... | Medium | `research/records/network.smb-require-dialect-3_1_1.review.json` |
| <a id="network.smb-require-signing-client"></a> `network.smb-require-signing-client` | SMB Client Signing Requirement | SMB signing helps Windows verify that network file-sharing traffic was not altered in transit. Requiring it makes the SMB client stricter... | Medium | `research/records/network.smb-require-signing-client.json` |
| <a id="network.smb-require-signing-server"></a> `network.smb-require-signing-server` | SMB Server Signing Requirement | SMB signing helps Windows verify that file-sharing traffic was not tampered with. Requiring it on the server side makes the machine stric... | Medium | `research/records/network.smb-require-signing-server.json` |
| <a id="network.smb-set-cipher-suite-order"></a> `network.smb-set-cipher-suite-order` | SMB Cipher Suite Order | This setting controls which SMB encryption cipher suites are preferred and in what order. It matters mostly for advanced SMB hardening or... | Medium | `research/records/network.smb-set-cipher-suite-order.review.json` |
<!-- TWEAK INDEX END -->
