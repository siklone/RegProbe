...
Instead of using Explorer search for every lookup, some operators prefer [`Everything`](https://www.voidtools.com/downloads/) because it keeps its own separate index and search workflow.
...
| --- | --- | --- | --- | --- |
| <a id="system.aero-shake"></a> `system.aero-shake` | Aero Shake Window Minimizing Gesture | Aero Shake is the gesture where you grab a window and shake it to minimize or restore other windows. This policy decides whether that ges... | High | `research/records/system.aero-shake.json` |
| <a id="system.bsod-disable-auto-reboot"></a> `system.bsod-disable-auto-reboot` | Automatic Restart on System Failure | Windows can automatically restart the PC after a blue screen. Turning that off gives you time to see the crash screen and stop code befor... | High | `research/records/system.bsod-disable-auto-reboot.json` |
| <a id="system.bsod-display-parameters"></a> `system.bsod-display-parameters` | Display Blue Screen Parameters | This tweak tries to show extra technical parameters on a blue screen. That can help advanced debugging, but most users will not benefit f... | High | `research/records/system.bsod-display-parameters.review.json` |
| <a id="system.disable-app-archiving"></a> `system.disable-app-archiving` | Automatic App Archiving | Windows can archive infrequently used apps to save space while keeping them restorable later. This policy decides whether that automatic... | High | `research/records/system.disable-app-archiving.json` |
| <a id="system.disable-auto-maintenance"></a> `system.disable-auto-maintenance` | Automatic Maintenance | Automatic Maintenance runs maintenance work like updates, scans, and housekeeping when Windows thinks the PC is idle. People disable it w... | High | `research/records/system.disable-auto-maintenance.review.json` |
| <a id="system.disable-background-gp-updates"></a> `system.disable-background-gp-updates` | Background Group Policy Refresh | Windows normally refreshes Group Policy in the background while the system is being used. This policy lets the device wait until logoff b... | High | `research/records/system.disable-background-gp-updates.json` |
| <a id="system.disable-clipboard-history"></a> `system.disable-clipboard-history` | Clipboard History and Cross-Device Clipboard Sync | Windows can keep a history of copied items and can sync clipboard data across devices. These two policy values decide whether those featu... | High | `research/records/system.disable-clipboard-history.json` |
| <a id="system.disable-clipboard-redirection"></a> `system.disable-clipboard-redirection` | RDP Clipboard Redirection | Remote Desktop can let text and files move between your local clipboard and the remote machine. This policy controls whether that copy an... | High | `research/records/system.disable-clipboard-redirection.json` |
| <a id="system.disable-fullscreen-optimizations"></a> `system.disable-fullscreen-optimizations` | Fullscreen Optimizations | Fullscreen Optimizations is a Windows feature that tries to blend fullscreen-style game performance with some modern desktop and presenta... | High | `research/records/system.disable-fullscreen-optimizations.review.json` |
| <a id="system.disable-game-recording-broadcasting"></a> `system.disable-game-recording-broadcasting` | Windows Game Recording and Broadcasting | Windows includes built-in game recording and broadcasting features. This policy decides whether those features are allowed on the device. | High | `research/records/system.disable-game-recording-broadcasting.json` |
| <a id="system.disable-jpeg-reduction"></a> `system.disable-jpeg-reduction` | Wallpaper JPEG Import Quality | Windows can reduce the quality of JPEG images when you set them as wallpaper. People change this when they want the wallpaper to stay as... | High | `research/records/system.disable-jpeg-reduction.review.json` |
| <a id="system.disable-restartable-apps"></a> `system.disable-restartable-apps` | Restart Apps After Sign-In | Windows can automatically save certain restartable apps and reopen them after you sign back in. People turn it off when they want a clean... | High | `research/records/system.disable-restartable-apps.review.json` |
| <a id="system.disable-scheduled-tasks"></a> `system.disable-scheduled-tasks` | Telemetry and Maintenance Scheduled Tasks Bundle | This tweak turns off several Windows scheduled tasks related to compatibility, telemetry, cleanup, feedback, and error-reporting areas. | High | `research/records/system.disable-scheduled-tasks.review.json` |
| <a id="system.disable-search-highlights-policy"></a> `system.disable-search-highlights-policy` | Search Highlights | Search Highlights are the dynamic bits Windows shows in the search area, such as seasonal or suggested content. This policy decides wheth... | High | `research/records/system.disable-search-highlights-policy.json` |
| <a id="system.disable-search-remote-queries"></a> `system.disable-search-remote-queries` | Remote Windows Search Queries | Windows Search can let other computers query this PC's search index over the network. This policy decides whether that remote index use i... | High | `research/records/system.disable-search-remote-queries.json` |
| <a id="system.disable-search-web-results"></a> `system.disable-search-web-results` | Web Results in Windows Search | Windows Search can send queries to the web and show web results in the search experience. This policy decides whether that web-connected... | High | `research/records/system.disable-search-web-results.review.json` |
| <a id="system.disable-service-splitting"></a> `system.disable-service-splitting` | Disable Service Splitting | Prevents services from being split into separate svchost processes. | Advanced | `app/Services/TweakProviders/SystemTweakProvider.cs#L22` |
| <a id="system.disable-shortcut-arrow"></a> `system.disable-shortcut-arrow` | Shortcut Arrow Overlay | Windows normally shows a small arrow on shortcut icons. This setting swaps that overlay resource so the arrow is no longer shown. | High | `research/records/system.disable-shortcut-arrow.json` |
| <a id="system.disable-startup-delay"></a> `system.disable-startup-delay` | Startup Program Delay | Windows can stagger startup apps instead of launching them all immediately. People disable the delay when they want those apps to start s... | High | `research/records/system.disable-startup-delay.review.json` |
| <a id="system.disable-storage-sense"></a> `system.disable-storage-sense` | Storage Sense Automatic Cleanup | Storage Sense is Windows' automatic cleanup feature for temporary and unneeded files. This policy decides whether the feature is allowed... | High | `research/records/system.disable-storage-sense.json` |
| <a id="system.disable-storage-sense-temp-cleanup"></a> `system.disable-storage-sense-temp-cleanup` | Storage Sense Temporary Files Cleanup | Storage Sense can automatically clean temporary files when Windows decides it is helpful. This policy controls that specific part of the... | High | `research/records/system.disable-storage-sense-temp-cleanup.json` |
| <a id="system.disable-store-open-with"></a> `system.disable-store-open-with` | Store in Open With | When Windows cannot find an app for a file type, it can offer a Store-based option in the Open With dialog. This policy decides whether t... | High | `research/records/system.disable-store-open-with.json` |
| <a id="system.dwm-disable-mpo"></a> `system.dwm-disable-mpo` | DWM: Disable Multiplane Overlay (MPO) | Disables MPO to avoid flicker or driver issues on some systems. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L140` |
| <a id="system.dwm-disable-overlay-min-fps"></a> `system.dwm-disable-overlay-min-fps` | DWM Overlay Minimum FPS Check | Windows can use hardware overlays for some content, and one of the checks looks at how fast the surface is animating. This setting turns... | High | `research/records/system.dwm-disable-overlay-min-fps.json` |
| <a id="system.enable-game-mode"></a> `system.enable-game-mode` | Game Mode | Game Mode is Windows' built-in gaming feature that tries to reduce interruptions and prioritize the current game. Most users either leave... | High | `research/records/system.enable-game-mode.review.json` |
| <a id="system.enable-hags"></a> `system.enable-hags` | Hardware-Accelerated GPU Scheduling | HAGS is a Windows graphics feature that lets the GPU take a more direct role in scheduling its work. It can help on some systems, but it... | High | `research/records/system.enable-hags.review.json` |
| <a id="system.enable-indexing-encrypted-items"></a> `system.enable-indexing-encrypted-items` | Indexing of Encrypted Files | Windows Search can be told to index encrypted files so their contents are searchable. That can make searching easier, but it also means W... | High | `research/records/system.enable-indexing-encrypted-items.json` |
| <a id="system.executive-additional-worker-threads"></a> `system.executive-additional-worker-threads` | Executive Additional Worker Threads | These values appear to control extra executive worker threads for internal critical and delayed work queues. They are low-level kernel-ad... | High | `research/records/system.executive-additional-worker-threads.json` |
| <a id="system.graphics-disable-overlays"></a> `system.graphics-disable-overlays` | Graphics: Disable Overlay Planes | Disables overlay planes to reduce composition issues in some configurations. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L127` |
| <a id="system.graphics-page-fault-debug-mode"></a> `system.graphics-page-fault-debug-mode` | Graphics: Page Fault Debug Mode | Restores the PageFaultDebugMode value to the documented default used by this tweak. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L152` |
| <a id="system.graphics-tdr-ddi-delay"></a> `system.graphics-tdr-ddi-delay` | Graphics TDR DDI Delay | Windows gives graphics-driver threads a limited amount of time to leave the driver during TDR recovery. This setting controls that delay. | High | `research/records/system.graphics-tdr-ddi-delay.json` |
| <a id="system.graphics-tdr-delay"></a> `system.graphics-tdr-delay` | Graphics TDR Delay | Windows uses TDR to detect when the GPU takes too long to respond. This setting controls the main timeout threshold before recovery starts. | High | `research/records/system.graphics-tdr-delay.json` |
| <a id="system.graphics-tdr-level"></a> `system.graphics-tdr-level` | Graphics TDR Level | Windows can react to GPU timeouts in different ways, such as disabling detection or recovering on timeout. This setting controls that rec... | High | `research/records/system.graphics-tdr-level.json` |
| <a id="system.graphics-tdr-limit-count"></a> `system.graphics-tdr-limit-count` | Graphics TDR Limit Count | Windows keeps track of how many GPU hangs occur within the TDR time window. This setting controls that count threshold. | High | `research/records/system.graphics-tdr-limit-count.json` |
| <a id="system.graphics-tdr-limit-time"></a> `system.graphics-tdr-limit-time` | Graphics TDR Limit Time | Windows measures GPU hang events within a time window when deciding how to respond to repeated timeouts. This setting controls that time... | High | `research/records/system.graphics-tdr-limit-time.json` |
| <a id="system.kernel-adjust-dpc-threshold"></a> `system.kernel-adjust-dpc-threshold` | Kernel: Adjust DPC Threshold | Sets the DPC threshold adjustment value to the documented default. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L18` |
| <a id="system.kernel-cache-aware-scheduling"></a> `system.kernel-cache-aware-scheduling` | Kernel: Cache-Aware Scheduling | Restores Windows' documented cache-aware scheduling default so the scheduler stays aligned with normal CPU cache topology behavior. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L90` |
| <a id="system.kernel-default-dynamic-hetero-cpu-policy"></a> `system.kernel-default-dynamic-hetero-cpu-policy` | Kernel: Default Dynamic Hetero CPU Policy | Returns Windows' hybrid CPU scheduling policy to the documented default used for heterogeneous core systems. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L102` |
| <a id="system.kernel-disable-low-qos-timer-resolution"></a> `system.kernel-disable-low-qos-timer-resolution` | Kernel: Disable Low QoS Timer Resolution | Disables low QoS timer resolution using the documented default value. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L114` |
| <a id="system.kernel-dpc-queue-depth"></a> `system.kernel-dpc-queue-depth` | Kernel: DPC Queue Depth | Sets the maximum DPC queue depth to the documented default. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L54` |
| <a id="system.kernel-dpc-watchdog-period"></a> `system.kernel-dpc-watchdog-period` | Kernel: DPC Watchdog Period | Sets the DPC watchdog timeout to the documented default (milliseconds). | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L66` |
| <a id="system.kernel-ideal-dpc-rate"></a> `system.kernel-ideal-dpc-rate` | Kernel: Ideal DPC Rate | Sets the target DPC rate per second to the documented default. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L30` |
| <a id="system.kernel-minimum-dpc-rate"></a> `system.kernel-minimum-dpc-rate` | Kernel: Minimum DPC Rate | Sets the minimum DPC rate threshold to the documented default. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L42` |
| <a id="system.kernel-serialize-timer-expiration"></a> `system.kernel-serialize-timer-expiration` | Kernel: Serialize Timer Expiration | Enables timer serialization using the documented default value. | Advanced | `app/Services/TweakProviders/SystemRegistryTweakProvider.cs#L78` |
| <a id="system.kernel-thread-dpc-enable"></a> `system.kernel-thread-dpc-enable` | Threaded DPCs | Threaded DPCs let some kernel deferred work run in a way that can reduce system latency compared with ordinary DPCs. Most normal systems... | High | `research/records/system.kernel-thread-dpc-enable.json` |
| <a id="system.kernel.disable-exception-chain-validation"></a> `system.kernel.disable-exception-chain-validation` | Disable Exception Chain Validation | DisableExceptionChainValidation is a low-level Session Manager Kernel registry value tied to exception-chain policy handling during early... | High | `research/records/system.kernel.disable-exception-chain-validation.json` |
| <a id="system.memory-clear-pagefile-at-shutdown"></a> `system.memory-clear-pagefile-at-shutdown` | Clear Page File at Shutdown | Windows can wipe the page file when the PC shuts down. This can reduce leftover data on disk, but it also makes shutdown take longer. | High | `research/records/system.memory-clear-pagefile-at-shutdown.json` |
| <a id="system.memory-disable-paging-executive"></a> `system.memory-disable-paging-executive` | Disable Paging Executive | This setting was used for older tracing and diagnosis scenarios to keep some kernel-mode code and drivers in RAM instead of letting Windo... | High | `research/records/system.memory-disable-paging-executive.json` |
| <a id="system.memory-large-system-cache-client"></a> `system.memory-large-system-cache-client` | Large System Cache | Windows historically had a switch between favoring normal applications and favoring a larger system cache. Most normal users should leave... | High | `research/records/system.memory-large-system-cache-client.review.json` |
| <a id="system.memory-nonpaged-pool-dynamic"></a> `system.memory-nonpaged-pool-dynamic` | Nonpaged Pool Size | Nonpaged pool is kernel memory that must always stay resident in RAM. This is a low-level legacy tuning area that normal users should not... | High | `research/records/system.memory-nonpaged-pool-dynamic.json` |
| <a id="system.memory-paged-pool-dynamic"></a> `system.memory-paged-pool-dynamic` | Paged Pool Size | Paged pool is kernel memory that Windows can page out when needed. This is a legacy low-level memory tuning area that most people should... | High | `research/records/system.memory-paged-pool-dynamic.json` |
| <a id="system.memory-registry-quota-default"></a> `system.memory-registry-quota-default` | Registry Size Limit / Registry Quota | Windows historically exposed a registry size limit tied to paged-pool memory. Modern systems normally manage this automatically, so the s... | High | `research/records/system.memory-registry-quota-default.review.json` |
| <a id="system.ntfs-disable-8dot3"></a> `system.ntfs-disable-8dot3` | NTFS 8.3 Short Name Creation | NTFS can create old-style short file names for compatibility. Disabling them can reduce some filesystem overhead, but some legacy softwar... | High | `research/records/system.ntfs-disable-8dot3.json` |
| <a id="system.ntfs-disable-last-access"></a> `system.ntfs-disable-last-access` | NTFS Last Access Time Updates | Windows can update a file or directory's last-access timestamp when it is accessed. Disabling these updates can reduce some disk overhead... | High | `research/records/system.ntfs-disable-last-access.json` |
| <a id="system.ntfs-enable-long-paths"></a> `system.ntfs-enable-long-paths` | Win32 Long Paths | Windows normally keeps many apps limited to shorter file paths. This setting allows long paths for apps that are built to support them. | High | `research/records/system.ntfs-enable-long-paths.json` |
| <a id="system.ntfs-reset-memory-usage"></a> `system.ntfs-reset-memory-usage` | NTFS Memory Usage | NTFS has an internal memory-usage tuning that affects its cache behavior. Most people should leave it at the default unless they have a p... | High | `research/records/system.ntfs-reset-memory-usage.json` |
| <a id="system.ntfs-reset-mft-zone"></a> `system.ntfs-reset-mft-zone` | NTFS MFT Zone Reservation | NTFS reserves some space for the master file table so it can grow with less fragmentation. This setting controls how large that reserved... | High | `research/records/system.ntfs-reset-mft-zone.json` |
| <a id="system.priority-control"></a> `system.priority-control` | Foreground Scheduling Priority / Win32PrioritySeparation | This setting changes how Windows gives extra CPU time to the foreground application compared with background work. It is a low-level sche... | High | `research/records/system.priority-control.review.json` |
| <a id="system.reliability-timestamp-enabled"></a> `system.reliability-timestamp-enabled` | Reliability Event Timestamping | Windows can timestamp reliability events on a cadence. This setting controls whether that timestamping behavior runs for the machine. | High | `research/records/system.reliability-timestamp-enabled.review.json` |
| <a id="system.services.disable-bluetooth-audio-gateway"></a> `system.services.disable-bluetooth-audio-gateway` | Bluetooth Audio Gateway Service | This service supports Bluetooth hands-free audio gateway features. Disabling it removes that capability. | High | `research/records/system.services.disable-bluetooth-audio-gateway.json` |
| <a id="system.services.disable-bluetooth-support"></a> `system.services.disable-bluetooth-support` | Bluetooth Support Service | This is the main Windows Bluetooth support service. It helps Windows discover, pair, and work with Bluetooth devices. | High | `research/records/system.services.disable-bluetooth-support.json` |
| <a id="system.services.disable-bluetooth-user-service"></a> `system.services.disable-bluetooth-user-service` | Bluetooth User Service | This per-user Bluetooth service supports Bluetooth features tied to each signed-in user session. Disabling it removes those per-user Blue... | High | `research/records/system.services.disable-bluetooth-user-service.json` |
| <a id="system.services.disable-connected-user-experiences"></a> `system.services.disable-connected-user-experiences` | Connected User Experiences and Telemetry Service | This service handles Windows telemetry and connected-user-experience data collection. Disabling it reduces diagnostic and usage reporting... | High | `research/records/system.services.disable-connected-user-experiences.json` |
| <a id="system.services.disable-print-device-configuration"></a> `system.services.disable-print-device-configuration` | Print Device Configuration Service | This service manages installation details for IPP and UP printers. Disabling it can stop that printer-installation support. | High | `research/records/system.services.disable-print-device-configuration.json` |
| <a id="system.services.disable-print-notifications"></a> `system.services.disable-print-notifications` | Print Notification Service | This service opens custom printer dialogs and handles printer notifications. Disabling it removes those printer notifications and related... | High | `research/records/system.services.disable-print-notifications.json` |
| <a id="system.services.disable-print-scan-broker"></a> `system.services.disable-print-scan-broker` | Print Scan Broker Service | This service supports secure privileged operations needed by the lower-privilege print spooler components. Disabling it removes that supp... | High | `research/records/system.services.disable-print-scan-broker.json` |
| <a id="system.services.disable-print-spooler"></a> `system.services.disable-print-spooler` | Print Spooler Service | Print Spooler handles print jobs and manages interaction with printers. Disabling it removes Windows printing support on the device. | High | `research/records/system.services.disable-print-spooler.json` |
| <a id="system.services.disable-print-workflow-user-service"></a> `system.services.disable-print-workflow-user-service` | Print Workflow User Service | This per-user service supports Print Workflow applications. Disabling it can stop those print workflow features and may prevent some prin... | High | `research/records/system.services.disable-print-workflow-user-service.json` |
| <a id="system.services.disable-sysmain"></a> `system.services.disable-sysmain` | SysMain Service | SysMain is Microsoft's documented background service for usage-pattern and preloading behavior. The record is deprecated because Microsof... | High | `research/records/system.services.disable-sysmain.review.json` |
| <a id="system.services.disable-wap-push-routing"></a> `system.services.disable-wap-push-routing` | WAP Push Message Routing Service | This service routes Wireless Application Protocol push messages and helps synchronize some device-management sessions. | High | `research/records/system.services.disable-wap-push-routing.review.json` |
| <a id="system.services.disable-windows-error-reporting"></a> `system.services.disable-windows-error-reporting` | Windows Error Reporting Service | Windows Error Reporting collects crash and failure details so Windows can report problems and sometimes suggest or surface fixes. | High | `research/records/system.services.disable-windows-error-reporting.review.json` |
| <a id="system.services.disable-windows-search"></a> `system.services.disable-windows-search` | Windows Search Service | Windows Search builds and maintains an index so file, email, and content searches can return indexed results without a full content crawl... | High | `research/records/system.services.disable-windows-search.json` |
| <a id="system.verbose-status-messages"></a> `system.verbose-status-messages` | Verbose Status Messages | Windows can either show simple loading screens or more detailed status text during startup, shutdown, sign-in, and sign-out. This policy... | High | `research/records/system.verbose-status-messages.json` |
| <a id="system.wait-to-kill-service-timeout"></a> `system.wait-to-kill-service-timeout` | Service Shutdown Timeout | Windows waits a set amount of time for a service to stop cleanly during shutdown. Lowering that timeout reduces how long Windows waits, b... | High | `research/records/system.wait-to-kill-service-timeout.review.json` |
<!-- TWEAK INDEX END -->
