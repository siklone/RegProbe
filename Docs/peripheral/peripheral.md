...
Disables system sounds and removes sound events. The retained notes used the same keys Windows disables:
```powershell
...
Everything listed below comes from retained historical research notes. Mistakes may exist, and some interpretations remain speculative until a current record-level re-audit exists. See the links below for reference.
...
Sample rate is the number of times per second that an audio signal is measured. Bit depth is the number of bits used to represent each sample; higher bit depth increases the available dynamic range. The most appropriate combination depends on the workload, while `44.1` kHz remains a common reference point in consumer and production audio.
...
For general desktop playback, `44.1` kHz at `16-bit` is the common baseline rather than a hard optimum.
...
Many gaming guides cluster around `800` or `1600` DPI, but this repo does not treat either value as a universal optimum. The linked captures are retained as examples of how very low DPI can degrade cursor behavior on some setups ([1](https://www.youtube.com/watch?v=mwf_F2VboFQ&t=458s), [2](https://www.youtube.com/watch?v=imYBTj2RXFs&t=274s)).
...
If the panel shows [ghosting](https://www.testufo.com/ghosting), retained external guidance often starts by testing a higher overdrive setting. If the panel then overshoots or shows inverse ghosting, step the setting back down. One workflow is to compare the monitor's response-time behavior against a reference review such as [RTINGS](https://www.rtings.com/) and keep the most stable setting rather than assuming the most aggressive preset is always appropriate.
...
| --- | --- | --- | --- | --- |
| <a id="audio.disable-beep"></a> `audio.disable-beep` | System Beep Driver | This affects the old system beep behavior driven by the Beep driver. Most people will never notice it unless they are cleaning up legacy... | Medium | `research/records/audio.disable-beep.review.json` |
| <a id="audio.disable-spatial-audio"></a> `audio.disable-spatial-audio` | Disable Spatial Audio | Sets the DisableSpatialOnLowLatency audio policy value to disable spatial audio on the low-latency path. | Advanced | `app/Services/TweakProviders/AudioTweakProvider.cs#L22` |
| <a id="audio.disable-system-sounds"></a> `audio.disable-system-sounds` | Disable System Sounds | Clears the default sound events for common system sounds. | Safe | `app/Services/TweakProviders/AudioTweakProvider.cs#L34` |
| <a id="audio.show-disconnected-devices"></a> `audio.show-disconnected-devices` | Show Disconnected Audio Devices | This makes disconnected audio devices appear in the classic Sound control panel. The evidence only proves the DeviceCpl preference is rea... | Medium | `research/records/audio.show-disconnected-devices.review.json` |
| <a id="audio.show-hidden-devices"></a> `audio.show-hidden-devices` | Show Hidden Audio Devices | This makes hidden audio devices appear in the classic Sound control panel. The evidence only proves the DeviceCpl preference is read by m... | Medium | `research/records/audio.show-hidden-devices.review.json` |
| <a id="peripheral.audio-disable-ducking"></a> `peripheral.audio-disable-ducking` | Disable Audio Ducking | Disables Windows automatic volume adjustment when making calls or using communication apps. Equivalent to 'Do nothing' in Sound settings. | Safe | `engine/Tweaks/Peripheral/AudioTweaks.cs#L24` |
| <a id="peripheral.autoplay-take-no-action"></a> `peripheral.autoplay-take-no-action` | AutoPlay Event Default Action | Windows can remember what to do when a device or media arrives. This tweak tries to change those remembered choices so common AutoPlay ev... | Medium | `research/records/peripheral.autoplay-take-no-action.review.json` |
| <a id="peripheral.disable-autoplay"></a> `peripheral.disable-autoplay` | Disable AutoPlay | AutoPlay decides what Windows does when media or devices are connected. Turning it off stops Windows from automatically offering or launc... | Medium | `research/records/peripheral.disable-autoplay.review.json` |
| <a id="peripheral.disable-sticky-keys-prompt"></a> `peripheral.disable-sticky-keys-prompt` | Disable Sticky Keys Prompt | Sticky Keys can show a prompt when Shift is pressed repeatedly. Some users turn that off to avoid accidental popups while gaming or typin... | Medium | `research/records/peripheral.disable-sticky-keys-prompt.review.json` |
| <a id="peripheral.keyboard-disable-language-hotkey"></a> `peripheral.keyboard-disable-language-hotkey` | Disable Language Switch Hotkey | Disables the Ctrl+Shift and Alt+Shift language-switch hotkeys. | Safe | `engine/Tweaks/Peripheral/KeyboardTweaks.cs#L52` |
| <a id="peripheral.keyboard-optimize-repeat"></a> `peripheral.keyboard-optimize-repeat` | Set Keyboard Repeat and Cursor Blink Values | Sets keyboard repeat delay to 0, repeat speed to 31, and cursor blink interval to 900 ms. | Safe | `engine/Tweaks/Peripheral/KeyboardTweaks.cs#L27` |
| <a id="peripheral.mouse-disable-acceleration"></a> `peripheral.mouse-disable-acceleration` | Disable Enhanced Pointer Precision (Mouse Acceleration) | Disables Enhanced Pointer Precision by setting mouse speed and threshold values to their non-accelerated configuration. | Safe | `engine/Tweaks/Peripheral/MouseTweaks.cs#L55` |
| <a id="peripheral.mouse-disable-throttle"></a> `peripheral.mouse-disable-throttle` | Disable Mouse Throttling for Background Windows | Disables raw mouse input throttling for background windows and sets the associated throttle duration and leeway values. | Safe | `engine/Tweaks/Peripheral/MouseTweaks.cs#L27` |
<!-- TWEAK INDEX END -->
