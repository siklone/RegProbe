...
| --- | --- | --- | --- | --- |
| <a id="explorer.always-show-icons-never-thumbnails"></a> `explorer.always-show-icons-never-thumbnails` | Always Show Icons, Never Thumbnails | Explorer can show file icons instead of generating thumbnails. | Medium | `research/records/explorer.always-show-icons-never-thumbnails.review.json` |
| <a id="explorer.disable-low-disk-space-warning"></a> `explorer.disable-low-disk-space-warning` | Low Disk Space Warning | Windows can warn you when a drive is running low on space. People sometimes turn these warnings off on lab machines or kiosk-like systems... | Medium | `research/records/explorer.disable-low-disk-space-warning.json` |
| <a id="explorer.disable-taskbar-chat"></a> `explorer.disable-taskbar-chat` | Taskbar Chat Icon | Windows 11 can show a Chat icon on the taskbar. People often hide it when they do not use Chat or want a cleaner taskbar. | Medium | `research/records/explorer.disable-taskbar-chat.json` |
| <a id="explorer.enable-explorer-compact-mode"></a> `explorer.enable-explorer-compact-mode` | Enable Explorer Compact View | Explorer compact view reduces spacing between items so more files fit on screen. Some users like the denser layout, while others prefer t... | Medium | `research/records/explorer.enable-explorer-compact-mode.review.json` |
| <a id="explorer.hide-empty-drives"></a> `explorer.hide-empty-drives` | Hide Empty Drives | Explorer can hide or show drives that do not currently have media present. | Medium | `research/records/explorer.hide-empty-drives.review.json` |
| <a id="explorer.launch-folder-windows-in-a-separate-process"></a> `explorer.launch-folder-windows-in-a-separate-process` | Launch Folder Windows in a Separate Process | Explorer can run folder windows in the same process or split them into separate Explorer processes. | Medium | `research/records/explorer.launch-folder-windows-in-a-separate-process.review.json` |
| <a id="explorer.show-compressed-and-encrypted-files-in-color"></a> `explorer.show-compressed-and-encrypted-files-in-color` | Show Compressed and Encrypted Files in Color | File Explorer can color compressed and encrypted NTFS items so they stand out visually. | Medium | `research/records/explorer.show-compressed-and-encrypted-files-in-color.review.json` |
| <a id="explorer.show-drive-letters-first"></a> `explorer.show-drive-letters-first` | Show Drive Letters First | Explorer can show drive letters ahead of the drive name, or leave that presentation disabled. | Medium | `research/records/explorer.show-drive-letters-first.review.json` |
| <a id="explorer.show-file-extensions"></a> `explorer.show-file-extensions` | Show File Extensions | File Explorer can hide or show file extensions like .txt, .zip, or .exe. Many advanced users prefer to always see them. | Medium | `research/records/explorer.show-file-extensions.review.json` |
| <a id="explorer.show-full-path"></a> `explorer.show-full-path` | Show Full Path in Explorer | Explorer can show the full folder path instead of a shorter title. Some advanced users prefer that when they navigate deep directories. | Medium | `research/records/explorer.show-full-path.review.json` |
| <a id="explorer.show-hidden-files"></a> `explorer.show-hidden-files` | Show Hidden Files and Folders | File Explorer can show or hide files and folders marked with the hidden attribute. Power users often enable this to inspect more of the f... | Medium | `research/records/explorer.show-hidden-files.review.json` |
| <a id="explorer.show-info-tips"></a> `explorer.show-info-tips` | Show Explorer Info Tips | File Explorer can show or suppress pop-up descriptions for folder and desktop items when you hover them. | Medium | `research/records/explorer.show-info-tips.review.json` |
| <a id="explorer.show-protected-operating-system-files"></a> `explorer.show-protected-operating-system-files` | Show Protected Operating System Files | File Explorer can hide or reveal protected operating system files. Most people should leave them hidden, but advanced troubleshooting som... | Medium | `research/records/explorer.show-protected-operating-system-files.review.json` |
| <a id="explorer.show-recent-items"></a> `explorer.show-recent-items` | Show Recent Items In Home | Explorer Home can show or hide recently used files. | Medium | `research/records/explorer.show-recent-items.review.json` |
| <a id="explorer.show-status-bar"></a> `explorer.show-status-bar` | Show Status Bar | Explorer can show or hide its status bar at the bottom of the window. | Medium | `research/records/explorer.show-status-bar.review.json` |
| <a id="explorer.show-type-overlay"></a> `explorer.show-type-overlay` | Display File Icons On Thumbnails | Explorer can show or hide file icons on top of thumbnails. | Medium | `research/records/explorer.show-type-overlay.review.json` |
| <a id="explorer.taskbar-alignment-left"></a> `explorer.taskbar-alignment-left` | Taskbar Alignment | Windows 11 can keep the taskbar icons centered or move them to the left. Some people prefer the older left-aligned layout. | Medium | `research/records/explorer.taskbar-alignment-left.review.json` |
| <a id="visibility.default-account-picture"></a> `visibility.default-account-picture` | Default Account Picture for All Users | Windows normally lets each user choose their own account picture. This policy forces every user on the device to use the default account... | Medium | `research/records/visibility.default-account-picture.json` |
| <a id="visibility.disable-acrylic-logon"></a> `visibility.disable-acrylic-logon` | Acrylic Blur on the Logon Background | Windows can blur the logon background with an acrylic effect. This policy decides whether that blur is turned off so the background stays... | Medium | `research/records/visibility.disable-acrylic-logon.json` |
| <a id="visibility.disable-common-control-animations"></a> `visibility.disable-common-control-animations` | Common Control and Window Animations | Windows can animate common controls and some window transitions. This policy decides whether those animations are turned off for the curr... | Medium | `research/records/visibility.disable-common-control-animations.json` |
| <a id="visibility.disable-first-signin-animation"></a> `visibility.disable-first-signin-animation` | First Sign-In Animation | Windows can show a first sign-in animation and, for Microsoft-account users, an opt-in prompt for services the first time someone signs i... | Medium | `research/records/visibility.disable-first-signin-animation.json` |
| <a id="visibility.disable-lock-screen"></a> `visibility.disable-lock-screen` | Lock Screen | Windows normally shows a lock screen before the sign-in screen. This policy decides whether that lock screen appears. | Medium | `research/records/visibility.disable-lock-screen.json` |
| <a id="visibility.disable-lock-screen-camera"></a> `visibility.disable-lock-screen-camera` | Lock Screen Camera Access | Windows can allow a camera to be used directly from the lock screen. This policy decides whether that lock-screen camera access can be en... | Medium | `research/records/visibility.disable-lock-screen-camera.json` |
| <a id="visibility.disable-lock-screen-changes"></a> `visibility.disable-lock-screen-changes` | Changing the Lock Screen and Logon Image | Windows normally lets users change the lock-screen and logon background image. This policy decides whether users are allowed to change th... | Medium | `research/records/visibility.disable-lock-screen-changes.json` |
| <a id="visibility.disable-lock-screen-motion"></a> `visibility.disable-lock-screen-motion` | Lock Screen Background Motion | Some devices can show a subtle motion effect on the lock-screen background. This policy decides whether that motion is prevented. | Medium | `research/records/visibility.disable-lock-screen-motion.json` |
| <a id="visibility.disable-lock-screen-slideshow"></a> `visibility.disable-lock-screen-slideshow` | Lock Screen Slideshow | Windows can run a slideshow on the lock screen instead of always showing a single image. This policy decides whether users can use that s... | Medium | `research/records/visibility.disable-lock-screen-slideshow.json` |
| <a id="visibility.disable-spotlight-action-center"></a> `visibility.disable-spotlight-action-center` | Windows Spotlight on Action Center | Windows can show Spotlight-driven suggestions or prompts in Action Center style notification surfaces. This policy decides whether that c... | Medium | `research/records/visibility.disable-spotlight-action-center.json` |
| <a id="visibility.disable-spotlight-desktop-collection"></a> `visibility.disable-spotlight-desktop-collection` | Spotlight Collection on Desktop | Windows can offer a Spotlight collection option for desktop backgrounds. This policy decides whether that Spotlight wallpaper option is a... | Medium | `research/records/visibility.disable-spotlight-desktop-collection.json` |
| <a id="visibility.disable-spotlight-features"></a> `visibility.disable-spotlight-features` | Windows Spotlight Features | Windows Spotlight powers various suggestions, tips, and promotional surfaces in Windows. This policy decides whether those Spotlight feat... | Medium | `research/records/visibility.disable-spotlight-features.json` |
| <a id="visibility.disable-spotlight-settings"></a> `visibility.disable-spotlight-settings` | Windows Spotlight on Settings | The Settings app can show Spotlight-based suggestions and highlights. This policy decides whether those Spotlight elements are allowed fo... | Medium | `research/records/visibility.disable-spotlight-settings.json` |
| <a id="visibility.disable-spotlight-third-party"></a> `visibility.disable-spotlight-third-party` | Third-Party Suggestions in Windows Spotlight | Windows Spotlight can include suggestions that point to third-party content or apps. This policy decides whether those suggestions are al... | Medium | `research/records/visibility.disable-spotlight-third-party.json` |
| <a id="visibility.disable-spotlight-welcome"></a> `visibility.disable-spotlight-welcome` | Windows Spotlight Welcome Experience | Windows can show a Spotlight-based welcome or introductory experience after updates or sign-in. This policy decides whether that welcome... | Medium | `research/records/visibility.disable-spotlight-welcome.json` |
| <a id="visibility.disable-wcn-wizards"></a> `visibility.disable-wcn-wizards` | Windows Connect Now Wizards | Windows Connect Now is an older wizard-based way to help set up some wireless routers and wireless devices. This policy decides whether t... | Medium | `research/records/visibility.disable-wcn-wizards.json` |
| <a id="visibility.disable-widgets"></a> `visibility.disable-widgets` | Widgets Feature | Windows Widgets can show a board with news, weather, and other feed content. This policy decides whether the Widgets feature is allowed o... | Medium | `research/records/visibility.disable-widgets.json` |
| <a id="visibility.disable-window-animations"></a> `visibility.disable-window-animations` | Window Animations | Windows can animate windows when they open, close, minimize, and restore. This policy decides whether those window animations are allowed. | Medium | `research/records/visibility.disable-window-animations.json` |
| <a id="visibility.force-classic-control-panel"></a> `visibility.force-classic-control-panel` | Control Panel Default View | Control Panel can open in either category view or the classic icons view. This policy chooses which one opens by default for the user. | Medium | `research/records/visibility.force-classic-control-panel.json` |
| <a id="visibility.hide-language-bar"></a> `visibility.hide-language-bar` | Language Bar Visibility | The language bar shows keyboard or input-language controls. This tweak hides that UI for the current user. | Medium | `research/records/visibility.hide-language-bar.review.json` |
| <a id="visibility.hide-most-used-apps"></a> `visibility.hide-most-used-apps` | Start Menu Most Used List | Windows can show a Most used list in Start to highlight apps you open often. This policy can force that list to be shown, hidden, or left... | Medium | `research/records/visibility.hide-most-used-apps.json` |
| <a id="visibility.hide-people-bar"></a> `visibility.hide-people-bar` | People Bar on the Taskbar | Windows used to offer a People Bar icon on the taskbar for quick contact access. This policy decides whether that feature is shown for th... | Medium | `research/records/visibility.hide-people-bar.json` |
| <a id="visibility.restore-classic-context-menu"></a> `visibility.restore-classic-context-menu` | Classic Context Menu on Windows 11 | Windows 11 normally shows the newer compact context menu. This tweak restores the older, fuller context menu style. | Medium | `research/records/visibility.restore-classic-context-menu.review.json` |
<!-- TWEAK INDEX END -->
