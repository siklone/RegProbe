...
| `powercfg /h /type full`           | **Change the hibernation file type to full.** Treat this cautiously on systems with less than 32GB of storage.                      |
...
| --- | --- | --- | --- | --- |
| <a id="power.control.allow-audio-to-enable-execution-required-power-requests"></a> `power.control.allow-audio-to-enable-execution-required-power-requests` | Allow Audio To Enable Execution Required Power Requests | This appears to be a kernel-adjacent Control Power setting tied to whether active audio can enable execution-required power requests. It... | Medium | `research/records/power.control.allow-audio-to-enable-execution-required-power-requests.json` |
| <a id="power.control.allow-system-required-power-requests"></a> `power.control.allow-system-required-power-requests` | Allow System Required Power Requests | This appears to be a kernel-adjacent Control Power setting tied to how the power manager treats system-required power requests. It is bei... | Medium | `research/records/power.control.allow-system-required-power-requests.json` |
| <a id="power.control.class1-initial-unpark-count"></a> `power.control.class1-initial-unpark-count` | Class1 Initial Unpark Count | Class1 Initial Unpark Count is a low-level power-manager registry value under the main Power key. | Medium | `research/records/power.control.class1-initial-unpark-count.json` |
| <a id="power.control.hiber-file-size-percent"></a> `power.control.hiber-file-size-percent` | Hiber File Size Percent | Hiber File Size Percent is a low-level power-manager value under the main Power key that appears to relate to hibernation file sizing beh... | Medium | `research/records/power.control.hiber-file-size-percent.json` |
| <a id="power.control.lid-reliability-state"></a> `power.control.lid-reliability-state` | Lid Reliability State | Lid Reliability State is a low-level power-manager registry value under the main Power key. | Medium | `research/records/power.control.lid-reliability-state.json` |
| <a id="power.control.mf-buffering-threshold"></a> `power.control.mf-buffering-threshold` | MF Buffering Threshold | MF Buffering Threshold is a low-level power-manager registry value under the main Power key. | Medium | `research/records/power.control.mf-buffering-threshold.json` |
| <a id="power.control.perf-calculate-actual-utilization"></a> `power.control.perf-calculate-actual-utilization` | Perf Calculate Actual Utilization | Perf Calculate Actual Utilization is a low-level power-manager registry value under the main Power key. | Medium | `research/records/power.control.perf-calculate-actual-utilization.json` |
| <a id="power.control.power-request-override-subtree"></a> `power.control.power-request-override-subtree` | Power Request Override Subtree | This is a low-level Control\Power subtree that appears to back override handling for power requests. | Medium | `research/records/power.control.power-request-override-subtree.json` |
| <a id="power.control.timer-rebase-threshold-on-drips-exit"></a> `power.control.timer-rebase-threshold-on-drips-exit` | Timer Rebase Threshold On Drips Exit | Timer Rebase Threshold On Drips Exit is a low-level power-manager registry value under the main Power key. | Medium | `research/records/power.control.timer-rebase-threshold-on-drips-exit.json` |
| <a id="power.disable-cpu-idle-states"></a> `power.disable-cpu-idle-states` | Disable CPU Idle States (C-States) | Writes the idle-state-related Control\\Power registry values used by this tweak. This changes power behavior and can increase power draw. | Advanced | `engine/Tweaks/Power/CPUPowerTweaks.cs#L56` |
| <a id="power.disable-cpu-parking"></a> `power.disable-cpu-parking` | Disable CPU Core Parking | Writes the core-parking-related Control\\Power registry values used by this tweak, including LatencyToleranceParked, LatencyToleranceSoft... | Safe | `engine/Tweaks/Power/CPUPowerTweaks.cs#L29` |
| <a id="power.disable-fast-startup"></a> `power.disable-fast-startup` | Disable Fast Startup (Hiberboot) | Disables Fast Startup so shutdown uses a full kernel teardown instead of reusing the hibernated kernel session on next boot. | Safe | `engine/Tweaks/Power/PowerSettingsTweaks.cs#L20` |
| <a id="power.disable-hibernation"></a> `power.disable-hibernation` | Disable Hibernation | Runs powercfg /hibernate off. This disables hibernation and removes hiberfil.sys. | Safe | `engine/Tweaks/Commands/Power/DisableHibernationTweak.cs#L15` |
| <a id="power.disable-network-power-saving.policy"></a> `power.disable-network-power-saving.policy` | Network Power and Multimedia Responsiveness | Writes the documented DisableTaskOffload and MMCSS SystemResponsiveness values while excluding the archived opaque NetworkThrottlingIndex... | Safe | `app/Services/TweakProviders/PowerTweakProvider.cs#L42` |
| <a id="power.disable-power-throttling"></a> `power.disable-power-throttling` | Disable Power Throttling | Disables Windows Power Throttling so background processes are not deprioritized by that policy surface. | Safe | `engine/Tweaks/Power/PowerSettingsTweaks.cs#L40` |
| <a id="power.disable-superfetch"></a> `power.disable-superfetch` | Disable Superfetch (SysMain) | Disables the SysMain service that preloads frequently used applications and maintains usage heuristics. | Safe | `engine/Tweaks/Commands/Performance/DisableSuperfetchTweak.cs#L14` |
| <a id="power.disable-windows-search"></a> `power.disable-windows-search` | Disable Windows Search | Disables the Windows Search indexing service. This reduces indexing activity but also slows file-search freshness and query coverage. | Advanced | `engine/Tweaks/Commands/Performance/DisableWindowsSearchTweak.cs#L13` |
| <a id="power.hide-hibernate-option"></a> `power.hide-hibernate-option` | Show Hibernate Option | Windows can show or hide Hibernate in the power menu. This policy decides whether that shortcut is visible. | Medium | `research/records/power.hide-hibernate-option.json` |
| <a id="power.hide-lock-option"></a> `power.hide-lock-option` | Show Lock Option | Windows can show or hide the Lock action in the user tile menu. This policy decides whether that shortcut is visible. | Medium | `research/records/power.hide-lock-option.json` |
| <a id="power.hide-sleep-option"></a> `power.hide-sleep-option` | Show Sleep Option | Windows can show or hide Sleep in the power menu. This policy decides whether that shortcut is visible. | Medium | `research/records/power.hide-sleep-option.json` |
| <a id="power.optimize-cpu-boost"></a> `power.optimize-cpu-boost` | Apply Processor Performance Registry Bundle | Writes the PerfBoostAtGuaranteed, HighPerfDurationBoot, LatencyToleranceDefault, and PerfCalculateActualUtilization values under Control\... | Safe | `engine/Tweaks/Power/CPUPowerTweaks.cs#L86` |
| <a id="power.optimize-gaming-network"></a> `power.optimize-gaming-network` | Set MMCSS Games Task Profile | Writes the MMCSS Games task-profile values for priority, scheduling category, SFIO priority, GPU priority, and affinity. | Safe | `engine/Tweaks/Power/NetworkAdapterPowerTweaks.cs#L50` |
| <a id="power.optimize-performance"></a> `power.optimize-performance` | Apply Grouped Power-Control Registry Bundle | Writes the grouped power-control registry values used by this tweak for timer coalescing, deep I/O coalescing, event processing, parked-c... | Advanced | `app/Services/TweakProviders/PowerTweakProvider.cs#L24` |
| <a id="power.session-watchdog-timeouts"></a> `power.session-watchdog-timeouts` | Session Manager Watchdog Timeouts | These values appear to control watchdog timeout behavior around directed power transitions. They are not normal user-facing power options... | Medium | `research/records/power.session-watchdog-timeouts.json` |
<!-- TWEAK INDEX END -->
