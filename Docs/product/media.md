...
These assets are intentionally lightweight and reproducible. The generator now emits both SVG source panels and PNG captures so the README can show something closer to the shipped shell without depending on a flaky VM screenshot lane.
...
That script refreshes the SVG source panels, PNG preview captures, and the README storyboard GIF.
