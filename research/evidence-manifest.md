...
| `system.kernel-dpc-watchdog-control-cluster` | draft | Class D | `research/records/system.kernel-dpc-watchdog-control-cluster.json` | - | `57b574c7434dc7e6630ca5e45906ccef487957b09a9d6d832c5d9ad910ff6432` |  | 7 |
| `system.kernel-dpc-watchdog-profile-cluster` | draft | Class D | `research/records/system.kernel-dpc-watchdog-profile-cluster.json` | - | `eea8087ca7cec174a732fb0cad9db406f8b71c4220cd39fdc8d2604a220b3155` |  | 9 |
...
- Source SHA256: `57b574c7434dc7e6630ca5e45906ccef487957b09a9d6d832c5d9ad910ff6432`
- Proof SHA256: ``

**Summary:** Draft Session Manager Kernel DPC watchdog control timeout cluster record. Repo docs list `DPCTimeout = 20000`, `DpcSoftTimeout = 20000`, and `DpcCumulativeSoftTimeout = 120000` under `HKLM\\SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Kernel`. Current-build KVM evidence now proves the full structural lane plus live values: `KeQueryDpcWatchdogConfiguration` reads the corresponding globals, `KiValidateDpcWatchdogConfiguration` backfills and validates them, `KeUpdateDpcWatchdogConfiguration` writes them from caller-supplied validated input, a PDB-backed Ghidra pass ties the writer to `NtSetSystemInformation`, a later class-focused live KD pass confirms the exact syscall call site plus admin-group gate and implies numeric arm `0xE4` from the dispatch chain, and a dedicated live value pass reads all three globals as `0` on the running build. The family is real, but the current build does not support treating the repo-doc values as unconditional live defaults or as proven registry-seeded state.
