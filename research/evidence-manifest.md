...
| `power.control.allow-audio-to-enable-execution-required-power-requests` | draft | Class D | `research/records/power.control.allow-audio-to-enable-execution-required-power-requests.json` | - | `7dbb69aba62665312c451a34b7cb721ddf25eed3699569d23f6d72d4b1c9b3da` |  | 7 |
| `power.control.allow-system-required-power-requests` | draft | Class D | `research/records/power.control.allow-system-required-power-requests.json` | - | `b7fb93bbbc38a491ba8143b08fb95595dfeb90c24a4e82f400dcc1cf7ef38898` |  | 6 |
...
- Source SHA256: `b7fb93bbbc38a491ba8143b08fb95595dfeb90c24a4e82f400dcc1cf7ef38898`
- Proof SHA256: ``

**Summary:** Draft Control Power record. `AllowSystemRequiredPowerRequests` sits under `HKLM\\\\SYSTEM\\\\CurrentControlSet\\\\Control\\\\Power`, has an explicit repo-doc default of `1`, is absent on the observed clean baseline, produced an exact current-build Unicode string hit in `ntoskrnl.exe`, and was later retained by source-enrichment as a docs-first kernel candidate with a `power-request-simulation` trigger recommendation. A later repo-native follow-up then exposed that same trigger family as a named guest Procmon profile and mega-trigger runtime alias. A later KVM local-KD pass then resolved live current-build `nt!PopPowerRequestConvertSystemToExecution = 1`, which aligns with the repo-doc default. The lane is strong enough for a schema-backed draft, but still lacks a current-build reader or seeding path and an exact runtime registry read.
