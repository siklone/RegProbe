...
The current package is still a useful watchdog research lead, but the bounded S1 chase is now repeating the same narrow outcome set: two dedicated Procmon runs both reached guest execution before falling over at export, the fallback S1 registry ETW lane still produced no exact target hit, and the value still lacks both a primary current-build documentation source and a proven non-default override model.
