...
The remaining gap for `AllowSystemRequiredPowerRequests` and `AllowAudioToEnableExecutionRequiredPowerRequests` is still environment-gated rather than runner-missing, but the blocker is now narrower than a generic guest-side qga fault. The repo has a narrow path-aware runtime lane for both records, the qemu guest-agent channel is attached on the active KVM session, the bootstrap ISO is restored, the guest-local installer can optionally bootstrap the Windows qemu guest agent, and a live keystroke fallback now exists for the focused KVM desktop. After qga MSI install, visible `VIOSERIALPORT` devices, guest-tools MSI install, reboot, direct foreground qga launches, an explicit service-state flip between `guest on` and `guest off`, and a host-side listener snapshot that still shows the qga path as a qemu-owned `LISTEN` socket, the live host-side state continues to fail with `ConnectionRefusedError(111, 'Connection refused')` on the qga path while the monitor socket remains materially different. On the current host, the next decisive step is therefore to debug qga-listener-specific host/libvirt/QEMU attach behavior rather than to keep treating the issue as missing runtime-runner plumbing, a simple Windows-side protocol exception, the guest merely having the port open, or the host simply lacking a listener.
