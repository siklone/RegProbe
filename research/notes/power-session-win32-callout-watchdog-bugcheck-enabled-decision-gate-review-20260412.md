...
The record remains useful as a watchdog-adjacent research lead, but the checked-in build still exposes it only as an adjacent sibling with no exact runtime read, no direct checked-in-build reader, no non-repo documentation source, and no proven non-default semantics. Those are material gaps for a bugcheck-related Session Manager value, and repeated bounded runtime retries are no longer a strong use of the lane.
