...
- The retained local-KD wildcard lineage pass returned no visible `nt!*PowerRequest*Reg*` symbols on the checked-in build.
...
- The visible checked-in-build setting-binding story is narrower than the repo power notes imply for the `Allow*ExecutionRequired*` pair.
...
- Combined with the generic query/store disassembly, this strengthens the checked-in conclusion: the visible execution-required power-request family is callback-, timer-, UMPO-, and in-memory-setting driven, while a `Control\Power` registry seeding path for the pair remains unproven on the checked-in build.
