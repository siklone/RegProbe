...
- this keeps the Phase 3 lane bounded: Freeware can remain available for manual GUI inspection, but it is not counted as operational headless cross-verification
- until that acceptance blocker is genuinely cleared, `IDA` is treated as pipeline-external and optional rather than as a required static lane
