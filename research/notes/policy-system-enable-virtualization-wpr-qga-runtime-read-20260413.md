...
This run also produced one tooling fix. The first 2026-04-13 pass overcaptured `EnableVirtualizationForInprocServer` and `EnableVirtualizationForInprocHandler` because the raw hit filter matched the target value name as a substring. [run-wpr-boot-registry-probe.ps1](../../scripts/vm/guest-tools/run-wpr-boot-registry-probe.ps1) now requires an exact quoted value-name match before a raw tracerpt registry line is retained or normalized.
