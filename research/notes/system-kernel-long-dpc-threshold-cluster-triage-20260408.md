...
- The lightweight `session-manager-kernel` runtime replay later kept both candidates in the residual `no-hit` hold after a write-and-reboot pass.
...
- new proof gained:
  ...
  - both values already survived a broad runtime replay and stayed `no-hit`
- narrowed conclusion:
