...
class VmKvmRegistryPolicyProbeTests(unittest.TestCase):
    ...
    def test_main_invalid_launcher_stage_reports_parse_error(self) -> None:
        with tempfile.TemporaryDirectory(dir=REPO_ROOT) as temp_root:
            upload_dir = Path(temp_root) / "upload"
            launcher_stage_path = upload_dir / "policy-probe-test-launcher-stage.json"

            def fake_launch(**kwargs):  # noqa: ANN003
                upload_dir.mkdir(parents=True, exist_ok=True)
                launcher_stage_path.write_text("{not-json", encoding="utf-8")
                return "qga"
        ...
        self.assertEqual(payload["status"], "error")
        self.assertEqual(payload["error_kind"], "registry-policy-launcher-stage-parse-error")
        self.assertEqual(payload["recovery_action"], "rerun-registry-policy-probe")
    ...
    def test_main_launcher_stage_stall_fast_fails(self) -> None:
        with tempfile.TemporaryDirectory(dir=REPO_ROOT) as temp_root:
            upload_dir = Path(temp_root) / "upload"
            launcher_stage_path = upload_dir / "policy-probe-test-launcher-stage.json"
            ...
            def fake_launch(**kwargs):  # noqa: ANN003
                upload_dir.mkdir(parents=True, exist_ok=True)
                launcher_stage_path.write_text(
                    json.dumps(
                        {
