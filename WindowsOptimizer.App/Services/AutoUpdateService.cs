...
public class AutoUpdateService : IDisposable
{
    ...
    public AutoUpdateService(string owner = "siklone", string repo = "WPF-Windows-optimizer-with-safe-reversible-tweaks")
    {
        _owner = owner;
        _repo = repo;
        _httpClient = new HttpClient
        {
            ...
            {
                { "User-Agent", "WindowsOptimizer-AutoUpdate" },
    ...
    public async Task<bool> DownloadAndInstallAsync(UpdateInfo update, IProgress<double>? progress = null)
    {
        ...
        try
        {
            var tempPath = Path.Combine(Path.GetTempPath(), $"WindowsOptimizer_Update_{update.LatestVersion}.exe");
