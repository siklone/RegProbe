...
param(
    ...
    [string]$GuestOutputRoot = 'C:\Tools\ValidationController\executive-worker-threads-procmon-bootlog',
    [string]$SnapshotName = 'baseline-20260327-regprobe-visible-shell-stable',
    [int]$PostBootSettleSeconds = 20,
    [switch]$CopyRawArtifacts
)
...
$probeName = "executive-worker-threads-procmon-bootlog-$stamp"
...
$hostPayloadPath = Join-Path $hostRoot 'executive-worker-threads-procmon-bootlog-payload.ps1'
$guestPayloadPath = Join-Path $guestRoot 'executive-worker-threads-procmon-bootlog-payload.ps1'
$hostLastBootQueryPath = Join-Path $hostRoot 'executive-query-lastboot.ps1'
$guestLastBootQueryPath = Join-Path $guestRoot 'executive-query-lastboot.ps1'
...
$guestPmlPath = Join-Path $guestRoot 'executive-worker-threads-procmon-bootlog.pml'
$guestCsvPath = Join-Path $guestRoot 'executive-worker-threads-procmon-bootlog.csv'
$guestHitsPath = Join-Path $guestRoot 'executive-worker-threads-procmon-bootlog.hits.csv'
$hostPmlPath = Join-Path $hostRoot 'executive-worker-threads-procmon-bootlog.pml'
$hostCsvPath = Join-Path $hostRoot 'executive-worker-threads-procmon-bootlog.csv'
$hostHitsPath = Join-Path $hostRoot 'executive-worker-threads-procmon-bootlog.hits.csv'
$repoPmlPlaceholderPath = Join-Path $repoRootOut 'executive-worker-threads-procmon-bootlog.pml.md'
...
$targetPathFragment = 'HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Executive'
$targetValueFragments = @(
    'AdditionalCriticalWorkerThreads',
    'AdditionalDelayedWorkerThreads',
    'UuidSequenceNumber'
)
$targetProcesses = @('System', 'smss.exe', 'services.exe')
    ...
    if ($summary['pml_exists']) {
        ...
        if ($summary['csv_exists']) {
            ...
            $hits = $rows | Where-Object {
                ...
                ($_.Path -match 'AdditionalCriticalWorkerThreads|AdditionalDelayedWorkerThreads|UuidSequenceNumber') -and
...
$summary = [ordered]@{
    ...
    procmon_pml_placeholder = "evidence/files/vm-tooling-staging/$probeName/executive-worker-threads-procmon-bootlog.pml.md"
    raw_artifacts_copied = [bool]$CopyRawArtifacts
    pml_captured = $false
    csv_captured = $false
    hits_captured = $false
    arm_summary = $null
...
try {
    ...
    try {
        Copy-FromGuest -GuestPath $guestHitsPath -HostPath $hostHitsPath
    }
    catch {
    }

    $summary.pml_captured = [bool](
        (Test-Path $hostPmlPath) -or
        ($summary.collect_summary -and $summary.collect_summary.pml_exists)
    )
    $summary.csv_captured = [bool](
        (Test-Path $hostCsvPath) -or
        ($summary.collect_summary -and $summary.collect_summary.csv_exists)
    )
    $summary.hits_captured = [bool](
        (Test-Path $hostHitsPath) -or
        ($summary.collect_summary -and $summary.collect_summary.hits_exists)
    )
...
$pmlPlaceholder = @(
    ...
    '',
    'Title: system.executive-additional-worker-threads Procmon boot log',
    '',
