...
def render_evidence_detail_drawer() -> str:
    ...
    body.append(tweak_card(268, 246, category="NETWORK", category_color=PALETTE["rail_purple"], name="Network Power + MMCSS", path=r"HKLM\...\Tcpip + Multimedia\SystemProfile", status="APP READY", tier="A", active=True))
    ...
    body.append(text(620, 322, "Network Power + MMCSS", size=25, weight=700))
    body.append(badge(1254, 296, "APPLY ALLOWED", fill_color=PALETTE["green_bg"], border_color=PALETTE["green"], text_color="#45bf78", width=136))
    body.append(badge(1402, 296, "TIER A", fill_color=PALETTE["gold"], border_color=PALETTE["gold"], text_color="#000000", width=80))
    body.append(text(620, 356, "WHAT IT DOES", size=10, weight=800, fill="#495360", family=MONO_STACK))
    body.append(paragraph(620, 384, ["Keeps TCP/IP task offload enabled with DisableTaskOffload = 0.", "Sets MMCSS SystemResponsiveness = 10 using documented value rules.", "Excludes opaque NetworkThrottlingIndex until primary semantics land."], size=15, fill=PALETTE["soft"], line_height=24))
    ...
    body.append(paragraph(646, 594, ["Microsoft docs cover the two app-ready values.", "The app writes only the narrowed documented bundle.", "Rollback is tied to the same two-value contract."], size=14, fill=PALETTE["soft"], line_height=22))
    ...
    body.append(paragraph(1104, 594, ["No NetworkThrottlingIndex semantics claim.", "No benchmark win implied by key/value existence.", "Old mixed bundle stays archived as research debt."], size=14, fill=PALETTE["soft"], line_height=22))
    body.append(stat_bar(620, 724, "DOCS", "ready", fill_color=PALETTE["green"], progress=90))
    body.append(stat_bar(890, 724, "RUNTIME", "bounded", fill_color=PALETTE["amber"], progress=48))
    body.append(stat_bar(1160, 724, "SOURCE", "clear", fill_color=PALETTE["blue"], progress=76))
    return window_shell("".join(body), active_top="Tweaks", status_items=[("Apply allowed", "ok"), ("Opaque value excluded", "warning"), ("Runtime bounded", "info"), ("Rollback documented", "ok")])
