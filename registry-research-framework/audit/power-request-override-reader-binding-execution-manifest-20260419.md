...
That payload now carries the same verifier summary plus `ready_for_execute`, a compact `blockers` list, an explicit output contract preview, an operator checklist, and a `next_steps` block. The dry-run preview also spells out that verifier contract up front so the operator can see what the later execute/readiness payload must contain. If the bundle is ready, the next command is the normal pipeline execute path; if not, the follow-up path falls back to the verifier markdown summary for human triage.
...
- Its JSON and markdown outputs now include `ready_for_execute`, a compact summary block, an explicit `blockers` list, an operator checklist, and concrete next-step guidance.
- Expected JSON contract: `ready_for_execute`, `summary`, `blockers`, `operator_checklist`, `next_steps`
- JSON summary:
...
- The pipeline `--verify-only` payload also includes `ready_for_execute`, the verifier `blockers` list, an operator checklist, and a `next_steps` block so the operator does not have to infer whether the right move is execute, dry-run, or markdown triage.
