...
- Duration seconds: `60`
- Candidate id: `power.control.allow-system-required-power-requests`
...
```powershell
powershell -NoProfile -Command "New-Item -ItemType Directory -Force -Path 'C:\RegProbe-Diag\etw-stackwalk\wave4-allow-system-required-e2e' | Out-Null"
```
```powershell
xperf -stop
```
...
```powershell
xperf -on PROC_THREAD+LOADER+REGISTRY -stackwalk RegCreateKey+RegOpenKey+RegQueryKey+RegSetValue+RegQueryValue+RegDeleteValue+RegCloseKey -BufferSize 1024 -MinBuffers 64 -MaxBuffers 256 -f C:\RegProbe-Diag\etw-stackwalk\wave4-allow-system-required-e2e\wave4-allow-system-required-e2e.raw.etl
```
...
```powershell
xperf -d C:\RegProbe-Diag\etw-stackwalk\wave4-allow-system-required-e2e\wave4-allow-system-required-e2e.etl
```

### parse_xml

```powershell
tracerpt C:\RegProbe-Diag\etw-stackwalk\wave4-allow-system-required-e2e\wave4-allow-system-required-e2e.etl -o C:\RegProbe-Diag\etw-stackwalk\wave4-allow-system-required-e2e\wave4-allow-system-required-e2e.xml -of XML
```
...
```powershell
python3 scripts/vm-kvm/run-guest-etw-stackwalk-capture.py --run-id wave4-allow-system-required-e2e --duration-seconds 60 --registry-path HKLM\SYSTEM\CurrentControlSet\Control\Power --value-name AllowSystemRequiredPowerRequests --ingest-to-repo --refresh-ghidra
```
