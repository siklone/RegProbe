...
$profiles = @(
    [ordered]@{
        id = 'guest-restart-roundtrip-bonc-lead3'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
    ...
    [ordered]@{
        id = 'guest-restart-roundtrip-bonc-lead10'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
        boot_mode = 'guest-restart'
        break_on_connect_mode = 'bonc'
    ...
    [ordered]@{
        id = 'guest-restart-roundtrip-bonc-lead20'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
        boot_mode = 'guest-restart'
        break_on_connect_mode = 'bonc'
    ...
    [ordered]@{
        id = 'guest-restart-roundtrip-none-lead10'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
        boot_mode = 'guest-restart'
    ...
    [ordered]@{
        id = 'guest-restart-roundtrip-none-lead20'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
        boot_mode = 'guest-restart'
        break_on_connect_mode = 'none'
    ...
    [ordered]@{
        id = 'guest-restart-roundtrip-b-lead10'
        debugger_frontend = 'kd'
        trace_profile = 'roundtrip-once'
        boot_mode = 'guest-restart'
...
$variantsPath = Join-Path $auditRoot ("windbg-start-order-variants-{0}.json" -f $resolvedAuditDate)
$executionPath = Join-Path $auditRoot ("windbg-start-order-execution-{0}.json" -f $resolvedAuditDate)
$matrixPath = Join-Path $auditRoot ("windbg-start-order-matrix-{0}.json" -f $resolvedAuditDate)
$recoveryPath = Join-Path $auditRoot ("windbg-start-order-recovery-{0}.json" -f $resolvedAuditDate)
$notePath = Join-Path $notesRoot ("windbg-start-order-matrix-{0}.md" -f $resolvedAuditDate)

$executionEntries = New-Object System.Collections.ArrayList
$recoveryEntries = New-Object System.Collections.ArrayList

foreach ($profile in $profiles) {
    $bundlePath = Join-Path $auditRoot ("windbg-start-order-bundle-{0}-{1}.json" -f $profile.id, $resolvedAuditDate)
    $bundleArgs = @{
    ...
    $entry = [ordered]@{
        ...
        roundtrip_attempted = [bool]$summary.roundtrip_attempted
        roundtrip_ready = [bool]$summary.roundtrip_ready
        post_restart_roundtrip_success_count = [int]$summary.post_restart_roundtrip_success_count
    }
...
$healthyRoundtripCount = @($executionEntries | Where-Object { $_.roundtrip_ready -and $_.shell_recovered -and -not $_.fatal_system_error_observed }).Count
...
$matrix = [ordered]@{
    ...
    phase = 'start-order-matrix'
    audit_date = $resolvedAuditDate
    profile_count = $profiles.Count
    roundtrip_ready_count = @($executionEntries | Where-Object { $_.roundtrip_ready }).Count
    healthy_roundtrip_count = $healthyRoundtripCount
    ...
    current_status = if ($healthyRoundtripCount -gt 0) {
        'partial'
    }
    elseif (@($executionEntries | Where-Object { $_.roundtrip_ready }).Count -gt 0) {
        'roundtrip-boot-unsafe'
    }
    ...
    recommended_next_actions = @(
        'Prefer any variant that reaches roundtrip_ready without fatal_system_error_observed before widening to semantics.',
        'If every start-order variant stays attach-ok-command-not-executed, move next to reconnect-time command injection or pipe endpoint experiments.',
        'Keep WinDbg single-key arbitration paused until one start-order variant yields a healthy post-restart prompt.'
    )
}
...
$noteLines = @(
    ...
    ('- roundtrip-ready profiles: `{0}`' -f (@($executionEntries | Where-Object { $_.roundtrip_ready }).Count)),
    ('- healthy-roundtrip profiles: `{0}`' -f $healthyRoundtripCount),
...
foreach ($entry in $executionEntries) {
    $noteLines += ('- `{0}` -> status `{1}`, transport `{2}`, lead `{3}`, bonc `{4}`, roundtrip_ready `{5}`, shell_recovered `{6}`, fatal `{7}`' -f $entry.id, $entry.status, $entry.windbg_transport_state, $entry.debugger_attach_lead_seconds, $entry.break_on_connect_mode, $entry.roundtrip_ready, $entry.shell_recovered, $entry.fatal_system_error_observed)
}
$noteLines += @(
    '',
    '## Follow-Up',
