...
DEFAULT_SOURCE_JSON = AUDIT_ROOT / "power-kernel-symbol-hunt-result-ledger-autofill.json"
DEFAULT_SOURCE_MD = AUDIT_ROOT / "power-kernel-symbol-hunt-result-ledger-autofill.md"
...
def target_paths(run_id: str, *, audit_root: Path | None = None) -> tuple[Path, Path]:
    ...
    stem = f"power-kernel-symbol-hunt-result-ledger-{suffix}"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Promote ignored Power/Kernel symbol-hunt autofill ledgers into dated audit artifacts.")
