...
DEFAULT_GLOB = "operator96-low-noise-rerun-tranche*-20260510.json"

TRANCHE_RE = re.compile(r"operator96-low-noise-rerun-tranche-(\d+)-20260510\.json$")
...
def campaign_sort_key(path: Path) -> tuple[int, str]:
    if path.name == "operator96-low-noise-rerun-tranche-20260510.json":
        return (1, path.name)
    match = TRANCHE_RE.match(path.name)
    if match:
