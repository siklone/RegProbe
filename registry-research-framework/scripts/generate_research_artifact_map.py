...
def summarize_cleanup_ledger(repo_root: Path) -> tuple[str, dict[str, Any]]:
    ...
    return status_from_bool(ok, ok_label="no-delete-eligible", fail_label="review-delete-eligible"), {
        ...
        "review_inventory_count": summary.get("review_inventory_count", summary.get("total_items")),
        "delete_candidate_count": summary.get("delete_candidate_count"),
        "cleanup_candidate_count": summary.get("cleanup_candidate_count", summary.get("delete_candidate_count")),
        "retained_inventory_count": summary.get("retained_inventory_count"),
        "retained_not_candidate_count": summary.get("retained_not_candidate_count", summary.get("retained_inventory_count")),
        ...
        "candidate_semantics": payload.get(
            "candidate_semantics",
            "candidate means cleanup_status=delete-candidate. Review inventory and retained inventory are not deletion candidates.",
        ),
    }
...
def summarize_cleanup_retained_plan(repo_root: Path) -> tuple[str, dict[str, Any]]:
    ...
    return status_from_bool(ok, ok_label="retained-plan-ready", fail_label="review-delete-ready"), {
        ...
        "active_cleanup_action_count": summary.get("active_cleanup_action_count"),
        ...
        "source_of_record_retained_count": summary.get("source_of_record_retained_count"),
        "historical_audit_retained_count": summary.get("historical_audit_retained_count"),
        "archive_history_retained_count": summary.get("archive_history_retained_count"),
        "vm_rerun_required_count": summary.get("vm_rerun_required_count"),
        "raw_trace_backfill_required_count": summary.get("raw_trace_backfill_required_count"),
...
def build_artifact_map(repo_root: Path = REPO_ROOT) -> dict[str, Any]:
    ...
    artifacts = [
        static_artifact(
            ...
            avoid_when="You are evaluating unshipped custom registry value experiments or raw experiment records.",
            status="reference",
        ),
        static_artifact(
        ...
        static_artifact(
            ...
            audience="contributor, research",
            tier="canonical-research",
            purpose="Explains why custom registry value experiment records remain Contributor Lab observations instead of normal cards.",
            use_when="You need to decide whether any user-supplied key/value experiment may enter app cards.",
    ...
    summary = {
        ...
        "custom_value_legacy_artifact_prefix": "operator96",
        "custom_value_legacy_artifact_note": "operator96 is a historical filename prefix for the user-supplied custom registry value seed batch, not product branding.",
        ...
        "operator96_legacy_alias": True,
        "cleanup_delete_eligible_count": cleanup.get("delete_eligible_count"),
        "cleanup_candidate_count": cleanup.get("cleanup_candidate_count"),
        "cleanup_review_inventory_count": cleanup.get("review_inventory_count"),
        "cleanup_retained_inventory_count": cleanup.get("retained_inventory_count"),
        "cleanup_retained_not_candidate_count": cleanup.get("retained_not_candidate_count"),
        "cleanup_reference_migration_needed_count": retained_plan.get("reference_migration_needed_count"),
        "cleanup_active_action_count": retained_plan.get("active_cleanup_action_count"),
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Custom value legacy prefix: `{summary.get('custom_value_legacy_artifact_prefix')}` is historical filename context, not product branding.",
        f"- Cleanup candidates: `{summary.get('cleanup_candidate_count')}`; review inventory: `{summary.get('cleanup_review_inventory_count')}`; retained non-candidates: `{summary.get('cleanup_retained_not_candidate_count')}`.",
