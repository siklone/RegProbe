...
DEFAULT_SCOREBOARD_PATH = FRAMEWORK_ROOT / "audit" / "etw-stackwalk-reopen-readiness-scoreboard.json"
DEFAULT_OUTPUT_PATH = FRAMEWORK_ROOT / "audit" / "etw-stackwalk-reopen-readiness-scoreboard-check.json"
DEFAULT_MARKDOWN_PATH = FRAMEWORK_ROOT / "audit" / "etw-stackwalk-reopen-readiness-scoreboard-check.md"

if str(CURRENT_DIR) not in sys.path:
    sys.path.insert(0, str(CURRENT_DIR))

from generate_etw_stackwalk_reopen_readiness_scoreboard import build_reopen_readiness_scoreboard  # noqa: E402
from generate_etw_stackwalk_reopen_readiness_scoreboard import load_json  # noqa: E402
from generate_etw_stackwalk_reopen_readiness_scoreboard import portable_path  # noqa: E402
...
def compare_reopen_readiness_scoreboard(
    ...
    *,
    ...
    for key in ("source_reopen_decision_ledger_path", "ledger_status", "scoreboard_status"):
    ...
    for key in ("candidate_count", "ready_count", "blocked_count", "dominant_reason_counts"):
    ...
    return {
        ...
        "errors": errors,
        "scoreboard_status": surface.get("scoreboard_status"),
...
def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        "# ETW Stackwalk Reopen Readiness Scoreboard Check",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the ETW stackwalk reopen readiness scoreboard.")
    parser.add_argument("--ledger", type=Path, default=DEFAULT_LEDGER_PATH)
    parser.add_argument("--scoreboard", type=Path, default=DEFAULT_SCOREBOARD_PATH)
    ...
    surface = load_json(args.scoreboard)
    expected = build_reopen_readiness_scoreboard(
        ledger_payload,
        generated_utc=str(surface.get("generated_utc") or now_utc()),
    )
    payload = compare_reopen_readiness_scoreboard(surface, expected)
    payload["ledger_path"] = portable_path(args.ledger)
    payload["scoreboard_path"] = portable_path(args.scoreboard)
