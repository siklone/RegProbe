...
MANIFEST_JSON = AUDIT_ROOT / "power-kernel-symbol-hunt-execution-manifest-20260422.json"
HANDOFF_JSON = AUDIT_ROOT / "power-kernel-symbol-hunt-handoff-index-20260422.json"
REACQUIRE_PLAN_JSON = AUDIT_ROOT / "power-kernel-symbol-hunt-reacquire-plan-20260422.json"
...
def build_next_steps(payload: dict[str, Any]) -> dict[str, str]:
    if payload.get("status") == "ok":
        recommended_example = "python3 scripts/vm-kvm/run-power-kernel-symbol-hunt-pipeline.py"
        recommended_reason = "Bundle verifier passed; the normal pipeline execute path is ready."
    else:
        recommended_example = "python3 registry-research-framework/scripts/verify_power_kernel_symbol_hunt_bundle.py --markdown"
    ...
    return {
        ...
        "dry_run_example": "python3 scripts/vm-kvm/run-power-kernel-symbol-hunt-pipeline.py --dry-run",
        "verify_only_example": "python3 scripts/vm-kvm/run-power-kernel-symbol-hunt-pipeline.py --verify-only",
        "markdown_summary_example": "python3 registry-research-framework/scripts/verify_power_kernel_symbol_hunt_bundle.py --markdown",
        "skip_bundle_verifier_example": "python3 scripts/vm-kvm/run-power-kernel-symbol-hunt-pipeline.py --skip-bundle-verifier"
    }
...
def build_operator_checklist(payload: dict[str, Any]) -> list[str]:
    if payload.get("status") == "ok":
        return [
            ...
            "Run the normal pipeline execute path to collect the four retained KD artifacts.",
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        "# Power/Kernel Symbol Hunt Bundle Verification",
        "",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify that the Power/Kernel symbol-hunt bundle is structurally intact.")
