...
Status: schema and parser support for `caller_stack` is in place, and the KVM WPR runner can now fail stack-expected runs when no frames are present. The first xperf registry stackwalk profile is now captured in `registry-research-framework/config/etw-stackwalk-profiles.json`, and `generate_etw_stackwalk_capture_plan.py` renders the elevated Windows commands plus repo parse handoff. The matching guest helper lives at `scripts/vm/guest-tools/run-etw-registry-stackwalk-capture.ps1` for hosts with Windows Performance Toolkit available. ETL discovery candidates now preserve `caller_stack` and `caller_stack_frame_count` when the parser finds stack frames, so the signal survives into queue/scoring views. The remaining capture work is to run that helper on a Windows host/guest and land the first real stack-bearing bundle.
