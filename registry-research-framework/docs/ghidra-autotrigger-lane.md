...
The lane is intentionally split between discovery and execution. The checked-in discovery, dispatch planning, health reporting, and validation flow works locally. Real headless execution remains blocked by the host environment because this checked-in machine setup does not provide both `pwsh` and a runnable Ghidra install.
